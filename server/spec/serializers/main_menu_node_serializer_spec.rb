...
describe MainMenuNodeSerializer do
  ...
  describe 'as_json' do
    ...
    context "for menu item for a page" do
      ...
      it { is_expected.to have_json_path('data/url') }
      it { is_expected.to be_json_eql("\"/#{menu_item_with_page.page.url_slug}\"").at_path('data/url')}
    ...
    context "for menu item for a link" do
      ...
      it { is_expected.to have_json_path('data/name')}
      it { is_expected.to have_json_path('data/url') }
      it { is_expected.to be_json_eql("\"#{menu_item_without_page.path}\"").at_path('data/url')}
      it { is_expected.to have_json_path('data/type')}
